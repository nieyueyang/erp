...
var vm = new Vue({
    ...
    methods: {
        init: function () {
            var pageNum = $("#jqGrid").jqGrid('getGridParam', 'page') != null || 1;
            var pageSize = $("#jqGrid").jqGrid('getGridParam', 'rowNum') != null || 10;
            var options = {};
            options.method = "GET";
            options.param = {"name": this.q.name, "pageNum": pageNum, "pageSize": pageSize};
            //options.formatJSon = true;
            ajaxWrapper(baseURL + 'sys/user/queryForPage', options, function (result) {
                this.userList = result.data;
                $("#jqGrid")[0].addJSONData(result.data.list);
                $("#jqGrid").jqGrid('setGridParam', {
                    records: result.data.total,
                    page: result.data.total,

                    page: result.data.pageNum,
